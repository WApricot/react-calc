    ...
    render: function(){
        return (
            React.createElement("div", {className: "radio-element"}, 
                React.createElement("input", {type: "radio", id: this.props.id, name: this.props.name, "data-type": this.props.type, onChange: this.props.function()}), 
...
var RadiosGroup = React.createClass({displayName: "RadiosGroup",
    ...
    render: function() {
        var keys=this.props.keys,
            ...
            functionForItem = this.changeElement();
        var elements = typesData.map(function(type, index){
            return React.createElement(RadioElement, {type: keys[type], fullName: keys[type]['name'], name: name, id: name + index, function: functionForItem})
        });
        return (
...
var CalcBlock = React.createClass({displayName: "CalcBlock",
    getCurrentType: function(type, element){
        console.log(123);
        return $('.type-block').find('input:checked').data('type')
    },
    render:function(){
        return(
            React.createElement("div", {className: "calc-form-inner"}, 
                ...
                React.createElement(RadiosGroup, {keys: data['sheet'][this.getCurrentType() || 'smart']['colors'], name: 'colors-block'})
            )
        )
    }
